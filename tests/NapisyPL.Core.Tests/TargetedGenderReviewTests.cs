...
public sealed class TargetedGenderReviewTests
{
    ...
    [Fact]
    public async Task ReviewAsync_AllowsOnlyCandidateIdsToChange()
    {
        var handler = new CountingHandler("""{"choices":[{"message":{"content":"[{\"id\":2,\"text\":\"Nie powinno się zmienić\"}]"},"finish_reason":"stop"}]}""");
        using var http = new HttpClient(handler);
        var service = new LocalTargetedGenderReviewService(http, "http://127.0.0.1:17843/v1", "qwen3-1.7b");
        ...
        await Assert.ThrowsAsync<InvalidDataException>(() =>
            service.ReviewAsync(source, translated, new Dictionary<int, string?> { [1] = "SPEAKER_00", [2] = "SPEAKER_01" }));
    }
